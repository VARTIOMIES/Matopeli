...
class MatopeliGUI:
    ...
    def __init__(self, asetukset):
        ...
        self.__peli_ikkuna.title("Matopeli 2.0.1")
        ...
        self.__peliruutu = []
        self.__kentän_koko_x = asetukset["kentän_koko"]+5
        self.__kentän_koko_y = asetukset["kentän_koko"]
        for x in range(self.__kentän_koko_x):
            self.__peliruutu.append([])
            for y in range(self.__kentän_koko_y):
        ...
        for x in range(len(self.__peliruutu)):
            self.__peliruutu_labels.append([""] * self.__kentän_koko_y)
    ...
    def suunnan_vaihto(self, uusi_suunta):
        ...
        # Nappien käyttöön otto ja käytöstä poisto.
        if self.__suunta == "vasen" or self.__suunta == "oikea":
            self.__ylös_nappi.configure(state=NORMAL)
            self.__oikealle_nappi.configure(state=DISABLED)
            self.__alas_nappi.configure(state=NORMAL)
            self.__vasemmalle_nappi.configure(state=DISABLED)
        elif self.__suunta == "alas" or self.__suunta == "ylös":
            self.__ylös_nappi.configure(state=DISABLED)
            self.__oikealle_nappi.configure(state=NORMAL)
            self.__alas_nappi.configure(state=DISABLED)
            self.__vasemmalle_nappi.configure(state=NORMAL)
    ...
    def tick(self):
        ...
        if self.__elossa:
            if len(self.__mato) > self.__koko:
                ...
                self.__peliruutu_labels[x_perä][y_perä].configure(
                    image=self.__tyhjä_ruutu)
                self.__peliruutu_labels[x_perä][y_perä].update()
            ...
            for mato_ruutu in self.__mato:
                ...
                self.__peliruutu_labels[x][y].grid(column=x,
                                                   row=y)
    ...
    def liikkuminen(self):
        ...
        if self.__suunta == "ylös":
            if y == 0:
                y = self.__kentän_koko_y-1
            else:
                y -= 1
        elif self.__suunta == "oikea":
            if x == self.__kentän_koko_x-1:
                x = 0
            else:
                x += 1
        elif self.__suunta == "alas":
            if y == self.__kentän_koko_y-1:
                y = 0
            else:
                y += 1
        elif self.__suunta == "vasen":
            if x == 0:
                x = self.__kentän_koko_x-1
            else:
                x -= 1
    ...
    def päivitä_ruutu(self):
        ...
        Uudessa verisossa tätä metodia ei tarvita, vaan vastaava toiminnallisuus
        on toteutettu yksinkertaisimmilla tavoilla
        """
    ...
    def syöminen(self):
        ...
        if self.__madon_pää[0] == self.__herkku[0] and \
            ...
            # Lisätään madon nopeutta
            if self.__pisteet <= 30:
                self.__nopeus -= 10
            else:
                self.__nopeus -= 5
    ...
    def herkkujen_lisäys(self):
        ...
        while herkku_madossa:
            ...
            herkun_x = randint(0, self.__kentän_koko_x-1)
            herkun_y = randint(0, self.__kentän_koko_y-1)
        ...
        # Lisätään herkku myös näkyville
        self.__peliruutu_labels[herkun_x][herkun_y].configure(
            image=self.__herkku_ruutu)
        self.__peliruutu_labels[herkun_x][herkun_y].grid(column=herkun_x,
                                                         row=herkun_y)
...
class MatomenuGUI:
    ...
    def __init__(self):
        ...
        self.__menu_ikkuna.title("Matopeli 2.0.1")
        ...
        self.__pelikentän_koko_asetus = Scale(self.__asetus_frame,
                                              orient=HORIZONTAL,
                                              label="Pelikentän koko",
                                              from_=5,
                                              to=50,
                                              length=200,
                                              resolution=5
        ...
        self.__pelikentän_koko_asetus.set(30)
        ...
        self.__pelikentän_koko_asetus.pack()
    ...
    def aloitus(self):
        ...
        self.__asetukset["kentän_koko"] = self.__pelikentän_koko_asetus.get()
